...
(function ($) {
  ...
  function Plugin(element, options) {
    ...
    if (typeof options.mapOptions.colorRange === "string") {
      ...
      options.mapOptions.colorRange = overrideColorRange
        ? colorRange
        : defaults.colorRange;
    }
    ...
    if (!options.mapLayers || !options.mapLayers.length) {
      console.log(
        'Map disabled - please add "map_layers" in site configuration.',
      );
      return;
    }
  ...
  Plugin.prototype = {
    ...
    updateTitle: function () {
      ...
      if (this.modelHelpers.GRAPH_TITLE_FROM_SERIES) {
        ...
        newTitle = this.modelHelpers.getChartTitle(
          currentTitle,
          this.chartTitles,
          currentUnit,
          currentSeries,
      ...
      if (newTitle) {
        if (
          this.proxy === "proxy" ||
          this.proxySerieses.includes(currentSeries)
        ) {
    ...
    updatePrecision: function () {
      ...
      var currentSeries = this.disaggregationControls.getCurrentSeries(),
        currentUnit = this.disaggregationControls.getCurrentUnit();
      this._precision = this.modelHelpers.getPrecision(
        this.precisionItems,
        currentUnit,
        currentSeries,
      );
    ...
    getTooltipContent: function (feature) {
      ...
      if (feature.properties.observation_attributes) {
        var obsAtts =
            feature.properties.observation_attributes[
              plugin.currentDisaggregation
            ][plugin.currentYear],
          footnoteNumbers = [];
        if (obsAtts) {
          Object.keys(obsAtts).forEach(function (field) {
            if (obsAtts[field]) {
              ...
              var footnoteNumber =
                plugin.allObservationAttributes[hashKey].footnoteNumber;
              footnoteNumbers.push(
                plugin.viewHelpers.getObservationAttributeFootnoteSymbol(
                  footnoteNumber,
                ),
              );
            }
    ...
    getData: function (props) {
      ...
      if (
        props.values &&
        props.values.length &&
        this.currentDisaggregation < props.values.length
      ) {
    ...
    replaceYearSlider: function () {
      ...
      $(this.yearSlider.getContainer()).insertAfter(
        $(this.disaggregationControls.getContainer()),
      );
      this.yearSlider._timeDimension.setCurrentTimeIndex(
        this.yearSlider._timeDimension.getCurrentTimeIndex(),
      );
    },
    ...
    init: function () {
      ...
      if (
        ...
        this.options.tileURL !== "undefined" &&
        this.options.tileURL != ""
      ) {
        L.tileLayer(this.options.tileURL, this.options.tileOptions).addTo(
          this.map,
        );
      }
      ...
      $.when.apply($, geoURLs).done(function () {
        ...
        for (var i = 0; i < geoJsons.length; i++) {
          ...
          var downloadButton = $("<a></a>")
            ...
            .attr(
              "title",
              translations.indicator.download_geojson_title +
                " - " +
                downloadLabel,
            )
            .attr(
            ...
            .text(
              translations.indicator.download_geojson + " - " + downloadLabel,
            );
          ...
          _.each(geoJson.features, function (feature) {
            if (
              ...
              feature.properties.values.length > 0
            ) {
              for (
                var valueIndex = 0;
                valueIndex < feature.properties.values.length;
                valueIndex++
              ) {
                var validEntries = _.reject(
                  Object.entries(feature.properties.values[valueIndex]),
                  function (entry) {
                ...
                availableYears[valueIndex] =
                  availableYears[valueIndex].concat(validKeys);
        ...
        for (
          var valueIndex = 0;
          valueIndex < minimumValues.length;
          valueIndex++
        ) {
          minimumValues[valueIndex] = _.reject(
            minimumValues[valueIndex],
            isMapValueInvalid,
          );
          maximumValues[valueIndex] = _.reject(
            maximumValues[valueIndex],
            isMapValueInvalid,
          );
          plugin.valueRanges[valueIndex] = [
            _.min(minimumValues[valueIndex]),
            _.max(maximumValues[valueIndex]),
          ];
        }
        ...
        plugin.currentYear =
          plugin.years[plugin.currentDisaggregation].slice(-1)[0];
        ...
        plugin.map.addControl(
          new L.Control.FullscreenAccessible({
            title: {
        ...
        plugin.disaggregationControls =
          L.Control.disaggregationControls(plugin);
      ...
      function finalMapPreparation() {
        ...
        plugin.yearSlider._timeDimension.setCurrentTimeIndex(
          plugin.yearSlider._timeDimension.getCurrentTimeIndex(),
        );
    ...
    featureShouldDisplay: function (feature) {
      ...
      display =
        display && typeof feature.properties.disaggregations !== "undefined";
      return display;
    },
...
Chart.register({
  ...
  afterUpdate: function (chart) {
    ...
    var ranges = _.chain(chart.data.datasets)
      .map("allData")
      .map(function (data) {
        return {
          min: _.findIndex(data, function (val) {
            return val !== null;
          }),
          max: _.findLastIndex(data, function (val) {
            return val !== null;
          }),
        };
      })
    ...
    if (dataRange) {
      ...
      chart.data.labels = chart.data.allLabels.slice(
        dataRange.min,
        dataRange.max + 1,
      );
...
Chart.register({
  ...
  afterDraw: function (chart) {
    if (chart.data.datasets.length === 0) {
      ...
      var lines = getTextLinesOnCanvas(
        ctx,
        translations.indicator.data_not_available,
        width,
      );
...
Chart.register({
  ...
  initElements: function () {
    ...
    if (window.innerWidth <= 768) {
      var mobileInstructions =
        translations.indicator.chart +
        ". " +
        translations.indicator.data_tabular_alternative;
      $(this.chart.canvas).html(
        '<span class="hide-during-image-download">' +
          mobileInstructions +
          "</span>",
      );
    } else {
      var keyboardInstructions =
        translations.indicator.data_keyboard_navigation;
      $("<span/>")
        .css("display", "none")
      ...
      $(this.chart.canvas)
        ...
        .html(
          '<span class="hide-during-image-download">Chart. ' +
  ...
  activate: function () {
    ...
    if (this.chart.config.type === "line") {
      ...
      activeElements.push({
        datasetIndex: this.currentDataset,
        ...
        if (observationAttributes && observationAttributes.length > 0) {
          label +=
            ...
            observationAttributes
              .map(helpers.getObservationAttributeText)
              .join(", ");
        }
...
var accessibilitySwitcher = function () {
  ...
  function setHighContrast() {
    $("body").removeClass("contrast-default").addClass("contrast-high");
    var title = translations.header.disable_high_contrast;
    var gaAttributes = opensdg.autotrack(
      "switch_contrast",
      "Accessibility",
  ...
  function setDefaultContrast() {
    $("body").removeClass("contrast-high").addClass("contrast-default");
    ...
    if (contrast == "high") {
      _.each($("img:not([src*=high-contrast])"), function (image) {
        ...
        if (switchThisImage) {
          $(image).attr(
            "src",
            $(image).attr("src").replace("img/", "img/high-contrast/"),
          );
        }
      });
    } else {
      ...
      _.each($("img[src*=high-contrast]"), function (goalImage) {
        $(goalImage).attr(
          "src",
          $(goalImage).attr("src").replace("high-contrast/", ""),
        );
      });
    }
  }
...
opensdg.chartColors = function (indicatorId) {
  ...
  this.goalNumber = parseInt(
    indicatorId.slice(indicatorId.indexOf("_") + 1, indicatorId.indexOf("-")),
  );
  this.goalColors = [
    [
      "e5243b",
  ...
  this.colorSets = {
    classic: ["7e984f", "8d73ca", "aaa533", "c65b8a", "4aac8d", "c95f44"],
    sdg: [
      "e5243b",
      "dda63a",
      "4c9f38",
  ...
  if (
    Object.keys(this.colorSets).indexOf(colorSet) == -1 ||
    (colorSet == "custom" && customColorList == null)
  ) {
    return this.colorSets["accessible"];
  }
  this.numberOfColors =
    numberOfColors > this.colorSets[colorSet].length ||
    numberOfColors == null ||
    numberOfColors == 0
      ? this.colorSets[colorSet].length
      : numberOfColors;
...
var indicatorModel = function (options) {
  var helpers = (function () {
    ...
    function nonFieldColumns() {
      ...
      var timeSeriesAttributes = [
        { field: "COMMENT_TS", label: "indicator.footnote" },
        {
    ...
    function getMatchesByUnitSeries(items, selectedUnit, selectedSeries) {
      ...
      if (matches.length === 0) {
        matches = items.filter(function (item) {
          var seriesMatch =
              item.series === selectedSeries && item.series && !item.unit,
    ...
    function dataHasUnitSpecificFields(fieldsUsedByUnit) {
      return !_.every(_.map(fieldsUsedByUnit, "fields"), function (fields) {
        return _.isEqual(
          _.sortBy(_.map(fieldsUsedByUnit, "fields")[0]),
          _.sortBy(fields),
        );
      });
    }
    ...
    function dataHasSeriesSpecificFields(fieldsUsedBySeries) {
      return !_.every(_.map(fieldsUsedBySeries, "fields"), function (fields) {
        return _.isEqual(
          _.sortBy(_.map(fieldsUsedBySeries, "fields")[0]),
          _.sortBy(fields),
        );
      });
    }
      ...
      if (edges.length > 0) {
        ...
        var fieldsNotInEdges = fieldItemStates
          ...
          .filter(function (field) {
            return !orderedEdges.includes(field);
          });
     ...
     * @return {Array} Field item states
     */
    function getUpdatedFieldItemStates(
      fieldItemStates,
      edges,
      selectedFields,
      validParentsByChild,
      ...
      getParentFieldNames(edges).forEach(function (parentFieldName) {
        if (selectedFieldNames.includes(parentFieldName)) {
          var childFieldNames = getChildFieldNamesByParent(
            edges,
            parentFieldName,
          );
            ...
            if (childFieldNames.includes(fieldItem.field)) {
              ...
              fieldItem.values.forEach(function (childValue) {
                ...
                selectedParent.values.forEach(function (parentValue) {
                  if (
                    validParentsByChild[fieldItem.field][
                      childValue.value
    ...
    function getFieldNames(fieldItems) {
      return fieldItems.map(function (item) {
        return item.field;
      });
    }
     ...
     */
    function fieldItemStatesForView(
      fieldItemStates,
      fieldsByUnit,
      selectedUnit,
      dataHasUnitSpecificFields,
      fieldsBySeries,
      selectedSeries,
      dataHasSeriesSpecificFields,
      selectedFields,
      edges,
      compositeBreakdownLabel,
    ) {
      var states = fieldItemStates.map(function (item) {
        return item;
      });
      if (dataHasUnitSpecificFields && dataHasSeriesSpecificFields) {
        states = fieldItemStatesForSeries(
          fieldItemStates,
          fieldsBySeries,
          selectedSeries,
        );
      ...
      return states.map(function (item) {
        ...
        if (
          item.field === "COMPOSITE_BREAKDOWN" &&
          compositeBreakdownLabel !== ""
        ) {
    ...
    function sortFieldsForView(fieldItemStates, edges) {
      if (edges.length > 0 && fieldItemStates.length > 0) {
        var parents = edges.map(function (edge) {
          return edge.From;
        });
        var children = edges.map(function (edge) {
          return edge.To;
        });
        var topLevelParents = [];
        ...
        children.forEach(function (child) {
          var currentParent = edges.find(function (edge) {
              return edge.To === child;
            }),
            currentChild = child;
          while (currentParent) {
            currentParent = edges.find(function (edge) {
              return edge.To === currentChild;
            });
            if (currentParent) {
        ...
        fieldItemStates.forEach(function (fieldItem) {
          if (
            topLevelParents.includes(fieldItem.field) ||
            typeof topLevelParentsByChild[fieldItem.field] === "undefined"
          ) {
        ...
        fieldItemStates.forEach(function (fieldItem) {
          if (
            typeof tempHierarchyHash[fieldItem.topLevelParent] === "undefined"
          ) {
            return;
          }
          if (fieldItem.topLevelParent !== "") {
            tempHierarchyHash[fieldItem.topLevelParent].children.push(
              fieldItem,
            );
          }
        });
     ...
     * @return {Array} Field item states
     */
    function fieldItemStatesForUnit(
      fieldItemStates,
      fieldsByUnit,
      selectedUnit,
    ) {
     ...
     * @return {Array} Field item states
     */
    function fieldItemStatesForSeries(
      fieldItemStates,
      fieldsBySeries,
      selectedSeries,
    ) {
      var fieldsBySelectedSeries = fieldsBySeries.filter(
        function (fieldBySeries) {
     ...
     * @return {Array} Field items
     */
    function selectMinimumStartingFields(
      rows,
    ...
    function validParentsByChild(edges, fieldItemStates, rows) {
      ...
      childFields.forEach(function (childField, fieldIndex) {
        ...
        childValues.forEach(function (childValue) {
          ...
          validParentsByChild[childField][childValue] =
            getUniqueValuesByProperty(parentField, rowsWithParentValues);
        });
      });
     ...
     * @return {Array} Field names
     */
    function getAllowedFieldsWithChildren(
      selectableFields,
      edges,
      selectedFields,
    ) {
    ...
    function getInitialAllowedFields(fieldNames, edges) {
      ...
      return fieldNames.filter(function (field) {
        return !children.includes(field);
      });
    }
    ...
    function sortFieldNames(fieldNames, dataSchema) {
      if (dataSchema && dataSchema.fields) {
        var schemaFieldNames = dataSchema.fields.map(function (field) {
          return field.name;
        });
        // If field names have been translated, we may need to use titles.
        if (
          schemaFieldNames.length > 0 &&
          !fieldNames.includes(schemaFieldNames[0])
        ) {
          schemaFieldNames = dataSchema.fields.map(function (field) {
            return field.title;
          });
        }
    ...
    function sortFieldValueNames(fieldName, fieldValues, dataSchema) {
      if (dataSchema && dataSchema.fields) {
        var fieldSchema = dataSchema.fields.find(function (x) {
          return x.name == fieldName;
        });
        // If field names have been translated, we may need to use titles.
        if (!fieldSchema) {
          fieldSchema = dataSchema.fields.find(function (x) {
            return x.title == fieldName;
          });
        }
        if (
          fieldSchema &&
          fieldSchema.constraints &&
          fieldSchema.constraints.enum
        ) {
          fieldValues.sort(function (a, b) {
            return (
              fieldSchema.constraints.enum.indexOf(a) -
              fieldSchema.constraints.enum.indexOf(b)
            );
          });
        } else {
          fieldValues.sort();
    ...
    function getGraphAnnotations(
      ...
      var annotations = getMatchesByUnitSeries(
        graphAnnotations,
        selectedUnit,
        selectedSeries,
      );
      if (graphTargetLines) {
        annotations = annotations.concat(
          getGraphTargetLines(graphTargetLines, selectedUnit, selectedSeries),
        );
      }
      if (graphSeriesBreaks) {
        annotations = annotations.concat(
          getGraphSeriesBreaks(graphSeriesBreaks, selectedUnit, selectedSeries),
        );
      }
      return annotations;
    }
     ...
     * @return {Array} Graph annotations objects, if any
     */
    function getGraphTargetLines(
      graphTargetLines,
      selectedUnit,
      selectedSeries,
    ) {
      return getMatchesByUnitSeries(
        graphTargetLines,
        selectedUnit,
        selectedSeries,
    ...
    function getDatasets(
      ...
      combinations.forEach(function (combination) {
        var filteredData = getDataMatchingCombination(
          data,
          combination,
          selectableFields,
        );
        if (filteredData.length > 0) {
          ...
          if (excess) {
            ...
            colorAssignment = getColorAssignmentByCombination(
              colorAssignments,
              combinationKey,
            );
    ...
    function getOpenColorInfo(colorAssignments, colors) {
      ...
      for (var i = 0; i < stripedStates.length; i++) {
        ...
        var assignedColors = colorAssignments
          .filter(function (colorAssignment) {
            return (
              colorAssignment.striped === stripedState &&
              colorAssignment.colorIndex !== null
            );
          })
    ...
    function observationAttributesTableFromDatasets(datasets, years) {
      return {
        data: years.map(function (year, index) {
          return [null].concat(
            datasets.map(function (ds) {
              return ds.observationAttributes[index]
                ? ds.observationAttributes[index]
                : [];
            }),
          );
        }),
    ...
    function getHeadlineTable(rows, selectedUnit) {
      return {
        title: "Headline data",
        headings: selectedUnit
          ? [YEAR_COLUMN, UNIT_COLUMN, VALUE_COLUMN]
          : [YEAR_COLUMN, VALUE_COLUMN],
        data: rows.map(function (row) {
          return selectedUnit
            ? [row[YEAR_COLUMN], row[UNIT_COLUMN], row[VALUE_COLUMN]]
            : [row[YEAR_COLUMN], row[VALUE_COLUMN]];
        }),
      };
    }
    ...
    function getHeadline(selectableFields, rows) {
      return rows
        ...
        .map(function (row) {
          ...
          return _.pickBy(row, function (val) {
            return val !== null;
          });
        });
    }
    ...
    function getPrecision(precisions, selectedUnit, selectedSeries) {
      var match = getMatchByUnitSeries(
        precisions,
        selectedUnit,
        selectedSeries,
      );
    ...
    function inputData(data) {
      ...
      if (
        ...
        opensdg.ignoredDisaggregations.length > 0
      ) {
    ...
    function inputEdges(edges) {
      ...
      if (
        ...
        edgesData = edgesData.filter(function (edge) {
          if (
            ignoredDisaggregations.includes(edge.To) ||
            ignoredDisaggregations.includes(edge.From)
          ) {
            return false;
          }
          return true;
      ...
      if (
        configuredObservationAttributes &&
        configuredObservationAttributes.length > 0
      ) {
        configuredObservationAttributesFlat =
          configuredObservationAttributes.map(function (att) {
            return att.field;
          });
        edgesData = edgesData.filter(function (edge) {
          if (
            configuredObservationAttributesFlat.includes(edge.To) ||
            configuredObservationAttributesFlat.includes(edge.From)
          ) {
            return false;
          }
          return true;
    ...
    function getAllObservationAttributes(rows) {
      ...
      configObsAttributes.forEach(function (field) {
        var attributeValues = Object.keys(_.groupBy(rows, field)).filter(
          function (value) {
    ...
    function deprecated(name) {
      return function () {
        console.log(
          "The " +
            name +
            " function has been removed. Please update any overridden files.",
        );
      };
    }
    ...
    return {
      ...
      observationAttributesTableFromDatasets:
        observationAttributesTableFromDatasets,
      sortFieldNames:
        typeof sortFieldNames !== "undefined" ? sortFieldNames : function () {},
      sortFieldValueNames:
        typeof sortFieldValueNames !== "undefined"
          ? sortFieldValueNames
          : function () {},
  ...
  this.initialiseUnits = function () {
    if (this.hasUnits) {
      this.units = helpers.getUniqueValuesByProperty(
        helpers.UNIT_COLUMN,
        this.data,
      );
      helpers.sortFieldValueNames(
        helpers.UNIT_COLUMN,
        this.units,
        this.dataSchema,
      );
      this.selectedUnit = this.units[0];
      this.fieldsByUnit = helpers.fieldsUsedByUnit(
        this.units,
        this.data,
        this.allColumns,
      );
      this.dataHasUnitSpecificFields = helpers.dataHasUnitSpecificFields(
        this.fieldsByUnit,
      );
    }
  };
  ...
  this.refreshSeries = function () {
    if (this.hasSerieses) {
      ...
      this.years = helpers
        .getUniqueValuesByProperty(helpers.YEAR_COLUMN, this.data)
        .sort();
      this.fieldsBySeries = helpers.fieldsUsedBySeries(
        this.serieses,
        this.data,
        this.allColumns,
      );
      this.dataHasSeriesSpecificFields = helpers.dataHasSeriesSpecificFields(
        this.fieldsBySeries,
      );
    }
  };
  ...
  this.initialiseFields = function () {
    this.fieldItemStates = helpers.getInitialFieldItemStates(
      this.data,
      this.edgesData,
      this.allColumns,
      this.dataSchema,
    );
    this.validParentsByChild = helpers.validParentsByChild(
      this.edgesData,
      this.fieldItemStates,
      this.data,
    );
    this.selectableFields = helpers.getFieldNames(this.fieldItemStates);
    this.allowedFields = helpers.getInitialAllowedFields(
  ...
  this.serieses = this.hasSerieses
    ? helpers.getUniqueValuesByProperty(helpers.SERIES_COLUMN, this.allData)
    : [];
  this.hasStartValues =
    Array.isArray(this.startValues) && this.startValues.length > 0;
  if (this.hasSerieses) {
    helpers.sortFieldValueNames(
      helpers.SERIES_COLUMN,
      this.serieses,
      this.dataSchema,
    );
    ...
    if (this.hasStartValues) {
      this.selectedSeries =
        helpers.getSeriesFromStartValues(this.startValues) ||
        this.selectedSeries;
    }
    this.refreshSeries();
  } else {
  ...
  this.allObservationAttributes = helpers.getAllObservationAttributes(
    this.allData,
  );
  ...
  this.updateFieldStates = function (selectedFields) {
    this.selectedFields = helpers.removeOrphanSelections(
      selectedFields,
      this.edgesData,
    );
    this.allowedFields = helpers.getAllowedFieldsWithChildren(
      this.selectableFields,
      this.edgesData,
      selectedFields,
    );
    this.fieldItemStates = helpers.getUpdatedFieldItemStates(
      this.fieldItemStates,
      this.edgesData,
      selectedFields,
      this.validParentsByChild,
  ...
  this.updateChartTitle = function () {
    this.chartTitle = helpers.getChartTitle(
      this.chartTitle,
      this.chartTitles,
      this.selectedUnit,
      this.selectedSeries,
  ...
  this.updateChartType = function () {
    this.graphType = helpers.getChartType(
      this.graphType,
      this.graphTypes,
      this.selectedUnit,
      this.selectedSeries,
  ...
  this.getData = function (options) {
    ...
    var headlineUnfiltered = helpers.getHeadline(
      this.selectableFields,
      this.data,
    );
    var headline;
    if (this.hasUnits && !this.hasSerieses) {
      ...
      headline = helpers.getDataBySeries(
        headlineUnfiltered,
        this.selectedSeries,
      );
    } else if (this.hasSerieses && this.hasUnits) {
    ...
    if (options.initial || options.changingSeries) {
      ...
      if (this.hasUnits) {
        ...
        if (this.hasStartValues) {
          var unitInStartValues = helpers.getUnitFromStartValues(
            this.startValues,
          );
      ...
      if (this.hasStartValues && !options.changingSeries) {
        startingFields = helpers.selectFieldsFromStartValues(
          this.startValues,
          this.selectableFields,
        );
      } else {
        if (headline.length === 0) {
          startingFields = helpers.selectMinimumStartingFields(
            this.data,
            this.selectableFields,
            this.selectedUnit,
          );
    ...
    if (options.initial || options.updateFields) {
      this.onFieldsComplete.notify({
        ...
        precision: helpers.getPrecision(
          this.precision,
          this.selectedUnit,
          this.selectedSeries,
        ),
    ...
    var filteredData = helpers.getDataBySelectedFields(
      this.data,
      this.selectedFields,
    );
    if (this.hasUnits) {
    ...
    var datasets = helpers.getDatasets(
      headline,
      filteredData,
      combinations,
      this.years,
      ...
      this.selectableFields,
      this.colorAssignments,
      this.allObservationAttributes,
    );
    var selectionsTable = helpers.tableDataFromDatasets(datasets, this.years);
    var observationAttributesTable =
      helpers.observationAttributesTableFromDatasets(datasets, this.years);
    ...
    this.onDataComplete.notify({
      ...
      selectedUnit: this.selectedUnit,
      selectedSeries: this.selectedSeries,
      graphLimits: helpers.getGraphLimits(
        this.graphLimits,
        this.selectedUnit,
        this.selectedSeries,
      ),
      stackedDisaggregation: this.stackedDisaggregation,
      graphAnnotations: helpers.getGraphAnnotations(
        this.graphAnnotations,
        this.selectedUnit,
        this.selectedSeries,
        this.graphTargetLines,
        this.graphSeriesBreaks,
      ),
      ...
      isProxy:
        this.proxy === "proxy" ||
        this.proxySerieses.includes(this.selectedSeries),
    });
  };
};
  ...
  this.initialise = function (
    ...
    $("#map").sdgMap({
      ...
      mapOptions: {
        ...
        maxZoom: 10,
        tileURL:
          "https://{s}.tile.jawg.io/{id}/{z}/{x}/{y}{r}.png?access-token={accessToken}",
        tileOptions: {
          id: "ed56748f-29f9-465e-9269-c715674ee5ff",
          accessToken:
            "yrcGMvxCkRD6zZBF4x3mnAT3OGQJstAj7JvG1iy1UjEdy6JFeDfnzKvXrTK07CxF",
          attribution:
...
var indicatorView = function (model, options) {
  ...
  var helpers = (function () {
    ...
    var PROXY_PILL =
      '<span aria-describedby="proxy-description" class="proxy-pill">' +
      translations.t("indicator.proxy") +
      "</span>";
    ...
    function initialiseFields(args) {
      ...
      if (fieldsContainValues) {
        ...
        if (!$("button#clear").length) {
          $(
            '<button id="clear" disabled="disabled" aria-disabled="true" class="disabled">' +
    ...
    function updateObservationAttributes(obsAttributes) {
      ...
      Object.values(obsAttributes).forEach(function (obsAttribute) {
        var label = getObservationAttributeText(obsAttribute),
          num = getObservationAttributeFootnoteSymbol(
            obsAttribute.footnoteNumber,
          );
        var $listItem = $(
          '<dt id="observation-footnote-title-' +
            num +
            '">' +
            num +
            '</dt><dd id="observation-footnote-desc-' +
            num +
            '">' +
            label +
            "</dd>",
    ...
    function getObservationAttributeText(obsAttribute) {
      ...
      var attributeConfig = _.find(
        configuredObsAttributes,
        function (configuredObsAttribute) {
    ...
    function updateUnitElements(selectedUnit) {
      ...
      if (hasUnit || fallback) {
        ...
        $("dd.data-controlled-footer-field.unit-from-data").text(
          translations.t(unitToDisplay),
        );
      } else {
    ...
    function updateIndicatorDataUnitStatus(unit) {
      var status =
        translations.indicator.announce_unit_switched + translations.t(unit);
    ...
    function updateIndicatorDataSeriesStatus(series) {
      var status =
        translations.indicator.announce_series_switched +
        translations.t(series);
    ...
    function getChartConfig(chartInfo) {
      ...
      if (typeof opensdg.chartTypes[chartType] === "undefined") {
        console.log(
          "Unknown chart type: " + chartType + '. Falling back to "line".',
        );
        chartType = "line";
      }
    ...
    function setPlotEvents(chartInfo) {
      ...
      createDownloadButton(
        ...
        chartInfo.selectedSeries,
        chartInfo.selectedUnit,
      );
      createSourceButton(chartInfo.shortIndicatorId, "#chartSelectionDownload");
      createIndicatorDownloadButtons(
        chartInfo.indicatorDownloads,
        chartInfo.shortIndicatorId,
        "#chartSelectionDownload",
      );
    ...
    function createPlot(chartInfo, helpers) {
      ...
      VIEW._chartInstance = new Chart(
        $(OPTIONS.rootElement).find("canvas"),
        chartConfig,
      );
      $(VIEW._legendElement).html(generateChartLegend(VIEW._chartInstance));
    }
    ...
    function updatePlot(chartInfo) {
      ...
      updateIndicatorDataViewStatus(
        VIEW._chartInstance.data.datasets,
        updatedConfig.data.datasets,
      );
      updateHeadlineColor(isHighContrast() ? "high" : "default", updatedConfig);

      if (chartInfo.selectedUnit) {
        updatedConfig.options.scales.y.title.text = translations.t(
          chartInfo.selectedUnit,
        );
      }
      ...
      updateGraphAnnotationColors(
        isHighContrast() ? "high" : "default",
        updatedConfig,
      );

      // The following is needed in our custom "rescaler" plugin.
      VIEW._chartInstance.data.allLabels =
        VIEW._chartInstance.data.labels.slice(0);

      VIEW._chartInstance.update();

      $(VIEW._legendElement).html(generateChartLegend(VIEW._chartInstance));
      updateChartDownloadButton(
        chartInfo.selectionsTable,
        chartInfo.selectedSeries,
        chartInfo.selectedUnit,
      );
    ...
    function updateGraphAnnotationColors(contrast, chartInfo) {
      if (chartInfo.options.plugins.annotation) {
        chartInfo.options.plugins.annotation.annotations.forEach(
          function (annotation) {
            if (contrast === "default") {
    ...
    function generateChartLegend(chart) {
      ...
      text.push(
        '<h5 class="sr-only">' +
          translations.indicator.plot_legend_description +
          "</h5>",
      );
      text.push(
        '<ul id="legend" class="legend-for-' + chart.config.type + '-chart">',
      );
      _.each(chart.data.datasets, function (dataset) {
        ...
        text.push(
          '<span class="swatch-inner" style="background-color: ' +
            dataset.borderColor +
            '"></span>',
        );
        text.push("</span>");
    ...
    opensdg.chartTypes.base = function (info) {
        ...
        options: {
          ...
          scales: {
            ...
            y: {
              ...
              title: {
                display: MODEL.selectedUnit
                  ? translations.t(MODEL.selectedUnit)
                  : MODEL.measurementUnit,
                text: MODEL.selectedUnit
                  ? translations.t(MODEL.selectedUnit)
                  : MODEL.measurementUnit,
                color: tickColor,
                font: {
            ...
            tooltip: {
              ...
              callbacks: {
                label: function (tooltipItem) {
                  return (
                    translations.t(tooltipItem.dataset.label) +
                    ": " +
                    alterDataDisplay(
                      tooltipItem.raw,
                      tooltipItem.dataset,
      ...
      if (info.graphAnnotations && info.graphAnnotations.length > 0) {
        ...
        var annotations = info.graphAnnotations.map(
          function (annotationOverrides) {
            var annotation = {};
            ...
            if (annotation.label && annotation.label.content) {
              annotation.label.content = translations.t(
            ...
            if (annotation.highContrast && annotation.highContrast.label) {
              if (annotation.highContrast.label.fontColor) {
                annotation.highContrast.label.color =
                  annotation.highContrast.label.fontColor;
              }
            }
            ...
            if (typeof annotation.defaultContrast === "undefined") {
              ...
              if (annotation.backgroundColor) {
                annotation.defaultContrast.backgroundColor =
                  annotation.backgroundColor;
              }
              if (annotation.label) {
                annotation.defaultContrast.label = {};
                if (annotation.label.color) {
                  annotation.defaultContrast.label.color =
                    annotation.label.color;
                }
                if (annotation.label.backgroundColor) {
                  annotation.defaultContrast.label.backgroundColor =
                    annotation.label.backgroundColor;
                }
                if (annotation.label.borderWidth) {
                  annotation.defaultContrast.label.borderWidth =
                    annotation.label.borderWidth;
                }
                if (annotation.label.borderColor) {
                  annotation.defaultContrast.label.borderColor =
                    annotation.label.borderColor;
                }
              }
            }
            return annotation;
    ...
    opensdg.chartTypes.bar = function (info) {
      ...
      if (info.stackedDisaggregation) {
        ...
        config.data.datasets.forEach(function (dataset) {
          ...
          if (
            typeof disaggregation[info.stackedDisaggregation] !== "undefined"
          ) {
            ...
            tooltip: {
              callbacks: {
                label: function (tooltipItem) {
                  ...
                  label +=
                    ": " +
                    opensdg.convertBinaryValue(tooltipItem.formattedValue);
                  return label;
                },
              },
            },
    ...
    function createSelectionsTable(chartInfo) {
      createTable(
        ...
        "#selectionsTable",
        chartInfo.isProxy,
        chartInfo.observationAttributesTable,
      );
      ...
      targetLines.forEach(function (targetLine) {
        ...
        $targetLines.append(
          "<dt>" +
            targetLineLabel +
            "</dt><dd>" +
            alterDataDisplay(targetLine.value, targetLine, "target line") +
            "</dd>",
        );
      });
    ...
    function createTable(
      ...
      if (table && tableHasData(table)) {
        ...
        var getHeading = function (heading, index) {
          var arrows =
            '<span class="sort"><i class="fa fa-sort"></i><i class="fa fa-sort-down"></i><i class="fa fa-sort-up"></i></span>';
          var button =
            '<span tabindex="0" role="button" aria-describedby="column-sort-info">' +
            translations.t(heading) +
            "</span>";
        ...
        $(el).append(
          $("<h3 />").text(translations.indicator.data_not_available),
        );
    ...
    function updateChartDownloadButton(table, selectedSeries, selectedUnit) {
      if (typeof VIEW._chartDownloadButton !== "undefined") {
        ...
        if (window.navigator && window.navigator.msSaveBlob) {
          ...
          VIEW._chartDownloadButton.on(
            "click.openSdgDownload",
            function (event) {
    ...
    function alterDataDisplay(value, info, context, additionalInfo) {
      ...
      if (obsAttributes.length > 0) {
        var obsAttributeFootnoteNumbers = obsAttributes.map(
          function (obsAttribute) {
            return getObservationAttributeFootnoteSymbol(
              obsAttribute.footnoteNumber,
            );
          },
        );
     ...
     * @return null
     */
    function createIndicatorDownloadButtons(
      indicatorDownloads,
      indicatorId,
      el,
    ) {
    ...
    return {
      ...
      getObservationAttributeFootnoteSymbol:
        getObservationAttributeFootnoteSymbol,
  ...
  $(document).ready(function () {
    $(OPTIONS.rootElement)
      ...
      .on("shown.bs.tab", function (e) {
        if ($(e.target).attr("href") == "#tableview") {
          setDataTableWidth(
            $(OPTIONS.rootElement).find("#selectionsTable table"),
          );
        } else {
    ...
    function toggleSidebar() {
      ...
      if (hideSidebar) {
        ...
        helpers.updateChartTitle(
          VIEW._dataCompleteArgs.chartTitle,
          VIEW._dataCompleteArgs.isProxy,
        );
        helpers.updateSeriesAndUnitElements(
          VIEW._dataCompleteArgs.selectedSeries,
          VIEW._dataCompleteArgs.selectedUnit,
        );
        helpers.updateUnitElements(VIEW._dataCompleteArgs.selectedUnit);
        helpers.updateTimeSeriesAttributes(
          VIEW._dataCompleteArgs.timeSeriesAttributes,
        );
      }
    }
  });
  ...
  MODEL.onSelectionUpdate.attach(function (sender, args) {
    ...
    $(".variable-selector").each(function (index, element) {
      ...
      var element = $(OPTIONS.rootElement).find(
      ...
      if (args.allowedFields.includes(currentField)) {
        ...
        $(element)
          .find("> button")
          .attr("aria-describedby", "variable-hint-" + currentField);
      }
    });
  });
  ...
  MODEL.onFieldsStatusUpdated.attach(function (sender, args) {
    _.each(args.data, function (fieldGroup) {
      ...
      var fieldGroupElement = $(OPTIONS.rootElement).find(
      ...
      if (!fieldGroup.hasData && !describedByCurrent.includes(noDataHintId)) {
        fieldGroupButton.attr(
          "aria-describedby",
          describedByCurrent + " " + noDataHintId,
        );
      } else {
        fieldGroupButton.attr(
          "aria-describedby",
          describedByCurrent.replace(noDataHintId, ""),
        );
      }
  ...
  if ($("#indicatorData").length) {
    ...
    if (domData.showdata) {
      $(".async-loading").each(function (i, obj) {
        $(obj).append(
          $("<img />")
            .attr("src", $(obj).data("img"))
            .attr("alt", translations.indicator.loading),
        );
      });
...
var indicatorSearch = function () {
  ...
  if (searchTerms !== null) {
    ...
    var searchWords = searchTermsToUse.split(" "),
      ...
      isIndicatorSearch =
        searchWords.length === 1 && indicatorIdParts.length >= 3;
    ...
    results.forEach(function (result) {
      ...
      doc.content = doc.content.replace(
        new RegExp("(" + escapeRegExp(searchTerms) + ")", "gi"),
    ...
    $("div.results").html(
      template({
        ...
        didYouMean:
          alternativeSearchTerms.length > 0 ? alternativeSearchTerms : false,
      }),
    );
  ...
  function getMatchedTerms(results) {
    ...
    results.forEach(function (result) {
      Object.keys(result.matchData.metadata).forEach(function (stemmedTerm) {
        Object.keys(result.matchData.metadata[stemmedTerm]).forEach(
          function (fieldName) {
            result.matchData.metadata[stemmedTerm][fieldName].unstemmed.forEach(
              function (unstemmedTerm) {
...
$(document).ready(function () {
  if (klaroConfig && klaroConfig.noAutoLoad !== true) {
    var cookieLink = $(
      '<li class="cookie-settings"><a role="button" tabindex="0">' +
        translations.cookies.cookie_settings +
        "</a></li>",
    );
        ...
        if (
          ...
          (u.add = function () {
            var t,
              ...
              i = e.length,
              s = !1;
            do ((t = e[n] + ""), -1 === c(this, t) && (this.push(t), (s = !0)));
            while (++n < i);
          ...
          try {
            ...
            (void 0 === g.number || -2146823252 === g.number) &&
...
!(function (e, n, o) {
  ...
  function t() {
    ...
    for (var f in i)
      if (i.hasOwnProperty(f)) {
        ...
        for (t = s(n.fn, "function") ? n.fn() : n.fn, a = 0; a < e.length; a++)
          ((l = e[a]),
            (c = l.split(".")),
            1 === c.length
              ? (Modernizr[c[0]] = t)
  ...
  function a(e) {
    ...
    Modernizr._config.enableClasses &&
      ((n += " " + o + e.join(" " + o)),
      f ? (c.className.baseVal = n) : (c.className = n));
  }
  var r = [],
    i = [],
    l = {
  ...
  (t(), a(r), delete l.addTest, delete l.addAsyncTest);
  ...
  L.Control.SelectionLegend = L.Control.extend({
    ...
    renderSwatches: function () {
      var controlTpl =
        ...
        "</div>";
      var swatchTpl =
        '<span class="legend-swatch" style="width:{width}%; background:{color};"></span>';
      var swatchWidth = 100 / this.plugin.options.colorRange.length;
      var swatches = this.plugin.options.colorRange
        .map(function (swatchColor) {
      ...
      return L.Util.template(controlTpl, {
        lowValue: this.plugin.alterData(
          opensdg.dataRounding(
            this.plugin.valueRanges[this.plugin.currentDisaggregation][0],
            context,
          ),
        ),
        highValue: this.plugin.alterData(
          opensdg.dataRounding(
            this.plugin.valueRanges[this.plugin.currentDisaggregation][1],
            context,
          ),
        ),
    ...
    update: function () {
      ...
      var valueRange =
        this.plugin.valueRanges[this.plugin.currentDisaggregation];
      selectionList.innerHTML = this.selections
...
(function () {
  ...
  L.Control.YearSlider = L.Control.TimeDimension.extend({
    ...
    _getDisplayDateFormat: function (date) {
      ...
      var match = this.options.years.find(function (y) {
        return y.time == time;
      });
      if (match) {
    ...
    _createButton: function (title, container) {
      if (title === "Date") {
        ...
        return L.Control.TimeDimension.prototype._createButton.call(
          this,
          title,
          container,
        );
    ...
    _createSliderTime: function (className, container) {
      var knob = L.Control.TimeDimension.prototype._createSliderTime.call(
          this,
          className,
          container,
        ),
      ...
      control._buttonBackward.setAttribute(
        "aria-label",
        control._buttonBackward.title,
      );
      control._buttonForward.title = translations.indicator.map_slider_forward;
      control._buttonForward.setAttribute(
        "aria-label",
        control._buttonForward.title,
      );
      ...
      knobElement.setAttribute(
        "aria-label",
        translations.indicator.map_slider_keyboard,
      );
...
(function () {
  ...
  L.Control.SearchAccessible = L.Control.Search.extend({
    ...
    _createInput: function (text, className) {
      var input = L.Control.Search.prototype._createInput.call(
        this,
        text,
        className,
      );
    ...
    _createTooltip: function (className) {
      var tooltip = L.Control.Search.prototype._createTooltip.call(
        this,
        className,
      );
    ...
    _handleSubmit: function (e) {
      ...
      if (
        (typeof e === "undefined" || e.type === "keyup") &&
        this._input.value === ""
      ) {
        return;
      }
      if (this._tooltip.childNodes.length > 0 && this._input.value !== "") {
        ...
        if (
          firstSuggestion !== userInput &&
          firstSuggestionLower.includes(userInputLower)
        ) {
  ...
  L.Control.DisaggregationControls = L.Control.extend({
    ...
    updateDisaggregations: function (startValues) {
      ...
      if (startValues && startValues.length > 0) {
        this.currentDisaggregation = this.getStartingDisaggregation(
          features,
          startValues,
        );
      ...
      this.hasDisaggregationsWithMultipleValuesFlag =
        this.hasDisaggregationsWithMultipleValues();
    },
    ...
    getStartingDisaggregation: function (features, startValues) {
      ...
      var disaggregations = features[0].properties.disaggregations,
        ...
        weighted = _.sortBy(
          disaggregations.map(function (disaggregation, index) {
    ...
    hasDissagregationsWithValues: function () {
      ...
      this.allDisaggregations.forEach(function (disaggregation) {
        if (
          disaggregation.values.length > 0 &&
          disaggregation.values[0] !== ""
        ) {
    ...
    hasDisaggregationsWithMultipleValues: function () {
      ...
      this.allDisaggregations.forEach(function (disaggregation) {
        if (
          disaggregation.values.length > 1 &&
          disaggregation.values[1] !== ""
        ) {
          hasDisaggregations = true;
        }
      });
    ...
    updateList: function () {
      ...
      if (this.hasDisaggregations) {
        var currentDisaggregation =
          this.disaggregations[this.currentDisaggregation];
        this.allDisaggregations.forEach(function (disaggregation) {
          var title = L.DomUtil.create("dt", "disaggregation-title"),
            definition = L.DomUtil.create("dd", "disaggregation-definition"),
            container = L.DomUtil.create("div", "disaggregation-container"),
    ...
    updateForm: function () {
        ...
        this.allDisaggregations.forEach(function (disaggregation) {
          var form = L.DomUtil.create(
            ...
            legend = L.DomUtil.create(
              "legend",
              "disaggregation-fieldset-legend",
            ),
            fieldset = L.DomUtil.create("fieldset", "disaggregation-fieldset"),
            field = disaggregation.field;
          ...
          disaggregation.values.forEach(function (value) {
            ...
            if (that.isDisaggegrationValidGivenCurrent(field, value)) {
              ...
              input.tabindex = 0;
              input.checked =
                value === currentDisaggregation[field] ? "checked" : "";
              var label = L.DomUtil.create("label", "disaggregation-label");
              label.innerHTML =
                value === "" ? translations.indicator.total : value;
              label.prepend(input);
              fieldset.append(label);
              input.addEventListener("change", function (e) {
                that.currentDisaggregation =
                  that.getSelectedDisaggregationIndex(field, value);
                that.updateForm();
              });
            }
          });
      ...
      var applyButton = L.DomUtil.create(
          "button",
          "disaggregation-apply-button",
        ),
        cancelButton = L.DomUtil.create(
          "button",
          "disaggregation-cancel-button",
        ),
        buttonContainer = L.DomUtil.create(
          "div",
    ...
    onAdd: function () {
      ...
      if (this.hasSeries || this.hasUnits || this.hasDisaggregations) {
        ...
        if (
          ...
          var containerOuter = L.DomUtil.create(
            "div",
    ...
    getAllSeries: function () {
      var seriesColumn = this.seriesColumn;
      if (
        typeof this.disaggregations[0][seriesColumn] === "undefined" ||
        !this.disaggregations[0][seriesColumn]
      ) {
        return [];
      }
      var allSeries = _.uniq(
        this.disaggregations.map(function (disaggregation) {
    ...
    getAllUnits: function () {
      ...
      if (
        typeof this.disaggregations[0][unitsColumn] === "undefined" ||
        !this.disaggregations[0][unitsColumn]
      ) {
        return [];
      }
      var allUnits = _.uniq(
    ...
    getAllDisaggregations: function () {
      var disaggregations = this.disaggregations,
        ...
        allDisaggregations = [];
      if (
        this.plugin.configObsAttributes &&
        this.plugin.configObsAttributes.length > 0
      ) {
        ...
        if (!invalidFields.includes(field) && validFields.includes(field)) {
          var sortedValues = valuesInOrder[field],
            item = {
              field: field,
              values: _.uniq(
                disaggregations.map(function (disaggregation) {
    ...
    getSelectedDisaggregationIndex: function (changedKey, newValue) {
      ...
      for (var i = 0; i < this.disaggregations.length; i++) {
        var disaggregation = this.disaggregations[i],
          keys = Object.keys(disaggregation);
        if (
          keys.includes(changedKey) &&
          disaggregation[changedKey] === newValue
        ) {
          return i;
        }
      }
    ...
    isDisaggegrationValidGivenCurrent: function (field, value) {
      var currentDisaggregation = Object.assign(
        {},
        this.disaggregations[this.currentDisaggregation],
      );
...
$(document).ready(function () {
  $('a[href="#top"]').prepend(
    '<svg class="app-c-back-to-top__icon" xmlns="http://www.w3.org/2000/svg" width="13" height="17" viewBox="0 0 13 17" aria-hidden="true" focusable="false"><path fill="currentColor" d="M6.5 0L0 6.5 1.4 8l4-4v12.7h2V4l4.3 4L13 6.4z"></path></svg>',
  );
});
